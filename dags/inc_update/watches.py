...
file_watch_history = inc_data_folder_path + "WatchHistory.txt"
...
def load(conn):
    ...
    df_file = pd.read_csv(file_watch_history, names=["CDC_FLAG","CDC_DSN","W_C_ID", "W_S_SYMB", "W_DTS", "W_ACTION"], sep="|")
    ...
    df_fact_watches.drop("CDC_FLAG", inplace=True, axis=1)
    df_fact_watches.drop("CDC_DSN", inplace=True, axis=1)
